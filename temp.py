...
def play_random_game():
    ...
    while not gameover(grid):
        ...
        grid, merge_gain, score_gain, moved = apply_action(grid, action)
        if moved:
            total_merges += merge_gain
            total_score += score_gain
            moves += 1
    return total_merges, total_score, max_tile(grid), moves
...
def mcts_search(root_state, num_simulations=50, c_param=1.414):
    ...
    for _ in range(num_simulations):
        ...
        if (not node.is_terminal()) and node.untried_actions:
            ...
            new_state, _, _, _ = apply_action(deepcopy(node.state), action)
        ...
        merges, reward, moves = rollout(node.state)

        #BACKPROPAGATION (update all node value's from expanded leaf node to root node)
        # for each node
        while node is not None:
            node.visits += 1 # increase visit by 1
            node.total_merges += merges
            node.total_reward += reward # increase total_reward by reward (metric used for evaluation)
            node.total_moves += moves
    ...
    with open(output_file, mode="w", newline="") as f:
        ...
        for config_idx, config in enumerate(experiments):
            ...
            for game_idx in range(games_per_setting):
                ...
                if method == "random":
                    final_merges, final_score, max_t, num_moves = play_random_game()
                elif method == "mcts":
                    final_merges, final_score, max_t, num_moves = play_mcts_game(
                ...
                print(
                    ...
                    f" merges={final_merges}, score={final_score}, max_tile={max_t}, "
